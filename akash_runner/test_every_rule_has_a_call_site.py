...
ADVISORY: dict[str, str] = {
    ...
    "check_provisioning_lives_in_just_akash.py": (
        "ADVISORY because it FAILS THE TWO REPOS THE STANDARD WAS WRITTEN ABOUT, by "
        "design — that is df-wiki#222 / gap G1: §1 of "
        "content/platform/akash-github-runners.md mandates runner provisioning workflows "
        "live ONLY in just-akash, the page's own opening sentence ('Two repos "
        "independently grew an akash-runner.yml') is still true on both mains, and the "
        "cannot-adopt explanation is refuted (Blazing-Back runs this suite cross-org "
        "today at a49af7142c97e7c4aab190bc0590cccbb843b4bc). Blast radius MEASURED "
        "2026-08-29 by running the rule against each origin/main tree, run-text, "
        "comments stripped: Blazing-Back @a5eb173e FAIL (akash-runner.yml, "
        "runner-time-to-ready.yml); blazing @1083b71e FAIL (akash-ci.yml, "
        "akash-integration-new.yml, akash-runner.yml); just-akash @4d8dc9b PASS as the "
        "verified publisher (runner-pool.yml's machinery is the standard there); "
        "df-cicd @1d2e7fe PASS; akash-lease-core @5e6e930 PASS; akash-github-runner "
        "@0b4da6d PASS. (The local checkout named 'DigitalFrontier-infra' IS "
        "Borduas-Holdings/Blazing-Back — same origin URL, same HEAD — so its row is the "
        "same row, not a seventh repo.) "
        "⚠ THE PROMOTION CONDITION NAMES THE PIN, because the obvious phrasing is "
        "UNSATISFIABLE: every suite consumer pins a checker SHA PREDATING this file "
        "(Blazing-Back and df-cicd pin a49af714… — df-cicd moved there in #190 on the "
        "day this rule was written; just-akash pins 6f1daff8…), so 'goes green on "
        "a consumer' cannot be reached by fixing consumers alone — the rule is ABSENT "
        "at the SHA they execute. Same unreachable shape already recorded for "
        "check_conformance_pin_agrees_with_checker_ref.py and "
        "check_listing_failure_is_loud.py. "
        "PROMOTE WHEN: (a) at least one consumer's `uses:@sha` AND `checker-ref` have "
        "both advanced to a SHA CONTAINING this file AND the rule has reported an "
        "EXECUTED verdict (PASS, findings, or NOT-JUDGEABLE) on a real conformance run "
        "there — shown to run, not merely merged; AND (b) every finding in the "
        "2026-08-29 measurement is either FIXED (machinery moved into just-akash and "
        "consumed via a pinned `uses:`) or EXEMPTED-WITH-A-REASON recorded in the "
        "standard — df-wiki#222's open question (does a repo-local CALLABLE reusable "
        "that others consume narrow the mandate?) is exactly such a reason, and it is "
        "settled in the wiki, not in a workflow file. A finding that is not true is a "
        "defect in the RULE and is never exemptible."
    ),
}
