...
ADVISORY: dict[str, str] = {
    ...
    "check_auto_update_pump_has_a_bound.py": (
        "ADVISORY because IN-SCOPE REPOS STILL FAIL, though no longer all of them. ⭐ UPDATED 2026-09-05: just-akash now PASSES — the landing gate landed in the shared `runner-pool.yml` (just-akash#262, merged e460729da), so every consumer of that workflow inherits it. Blazing-Back is the multi-file case this rule exists for: `akash-runner.yml` is bound (#1590, merged 99b615b6b) while `runner-time-to-ready.yml` is NOT, and before the per-container fix the first would have silenced the second and the repo would have read clean. blazing still fails on 3 in-scope files. This rule closes the half of the auto-update trade that "
        ...
        "(2026-09-05): a HEALTHY 2.336.0 runner, already 'Listening for Jobs' with NO "
        "deprecation message, took the mandated 2.337.0 update, exited, failed to "
        "deregister, and left `Could not load file or assembly 'System.Linq.Parallel'`. "
        "Both pumps share ONE precondition — unbounded restart of a container that can "
        "register but never work — and this rule requires that precondition be broken, by "
        "a landing gate (read `.version` back from the listing after the pool reports "
        "online) or a non-`always` restart policy. "
        "Blast radius MEASURED 2026-09-05 by running the rule against each workflows dir "
        "directly: just-akash FAIL (1 — runner-pool.yml); Blazing-Back FAIL (2 — "
        "akash-runner.yml AND runner-time-to-ready.yml); blazing FAIL (3 — akash-ci.yml, "
        "akash-integration-new.yml, akash-runner.yml); df-cicd, akash-github-runner and "
        "df-wiki NOT APPLICABLE (they define no runner container, which is not a pass). "
        "⭐ THE RULE IS NOT MERELY UNIVERSALLY-FAILING: Blazing-Back #1590's branch, which "
        "adds the deploy-time version gate, PASSES it (landing-gate found). That pair — "
        "fails main, passes the fix — is what makes it a discriminator rather than a "
        "complaint, and it is pinned in test_check_auto_update_pump_has_a_bound.py. "
        "⚠ THE PROMOTION CONDITION NAMES THE SHARED WORKFLOW, not just the consumers, "
        "because blazing owns the measured churn (1,732 org registrations on 2026-09-05, "
        "a 300-sample 300/300 offline and `version: null`, all `akash-*` prefixes) and "
        "provisions ONLY via just-akash's runner-pool.yml — so a gate landed in "
        "Blazing-Back's private fork of akash-runner.yml would leave the largest producer "
        "unguarded. PROMOTE WHEN: (a) just-akash/.github/workflows/runner-pool.yml carries "
        "a bound, so every consumer of the shared pool inherits it, AND (b) Blazing-Back "
        "and blazing have moved their conformance pin to a SHA containing this rule. "
        "Either half alone leaves the promotion unreachable."
    ),
